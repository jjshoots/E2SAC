...
class DDQN(nn.Module):
    ...
    def evidential_sample(self, q, uncertainty, label):
        # get what the agent would do
        action = self.infer(q, uncertainty)

        # compute the q of the agent's action
        expected_q = q.mean(dim=-1).squeeze()[action]

        # get upper and lower bound
        uncertain_q = expected_q + uncertainty
        upperbound, _ = torch.max(uncertain_q, dim=-1)
        upperbound = upperbound.squeeze()
        lowerbound, _ = torch.min(uncertain_q, dim=-1)
        lowerbound = lowerbound.squeeze()

        # probability that we use the label
        uncertainty = (upperbound[label] - lowerbound[action]) / expected_q.abs()

        prob_sample = (
            torch.clamp(
                uncertainty * self.confidence_lambda,
                min=self.exploration_epsilon,
                max=1.0,
            )
            .detach()
            .cpu()
        ...
        log = dict()
        log["evidential_prob"] = prob_sample
        log["uncertainty"] = uncertainty

        # with probability of prob_sample, we use the label
        if np.random.random_sample() < prob_sample:
            return torch.tensor(label), log
        else:
            return self.sample(q, uncertainty), log
