    ...
    while memory.count <= cfg.total_steps:
        ...
        while not env.ended:
            ...
            if memory.count < cfg.exploration_steps:
                ...
                # compute the q values using the mean action
                mean_act = output[0].detach()
                mean_act.requires_grad_(True)
                q_vals = net.critic(gpuize(obs, cfg.device).unsqueeze(0), mean_act)

                # compute upperbound q value
                q_mean = q_vals.mean(dim=-1)
                q_std = 0.5 * (q_vals[:, 0] - q_vals[:, 1]).abs()
                q_upper = q_mean + cfg.oac_beta * q_std

                # backward to get eq. 8 in https://arxiv.org/pdf/1910.12807.pdf
                # use that to get eq. 9
                q_upper.backward()
                act_grad = mean_act.grad.clone()
                mean_act = mean_act.detach()
                mean_act.requires_grad_(True)
                mean_act = mean_act + cfg.oac_delta / torch.norm(act_grad) * output[1] * act_grad
